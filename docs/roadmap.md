# Roadmap — build order

The to-do list for what to build next. Tick items as `[x]` when done; add notes inline as the real requirements of a step become clearer. Each step should leave the site buildable (`npx astro build` green).

- [ ] 1. Scaffold Astro project, base layout, SEO component + schema builders, global styles. ([seo.md](seo.md))
- [ ] 2. Sanity studio + schemas, seed with the two speakers and 3–4 topics. ([content-model.md](content-model.md))
- [ ] 3. Templates: front page, speaker index/detail, topic index/detail.
- [ ] 4. Booking page + form endpoint, kontakt, om-oss, FAQ. ([site-structure.md](site-structure.md) § Conversion)
- [ ] 5. Blog + case studies + testimonials templates.
- [ ] 6. sitemap/robots/llms.txt, OG image handling, 404, final Lighthouse pass. ([seo.md](seo.md) § Technical checklist)
- [ ] 7. Cloudflare Pages deploy + Sanity webhook.
